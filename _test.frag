...
void main() {
  ...
  vec3 c = curl(vec3(uv * 0.5, u_time * 0.05));
  float w = worley(vec3(uv * 100.0 * c.yz + c.x, c.x));
  vec3 v = voronoi(uv * 1.0, w * length(c) * c.r + t);
  vec3 p = palette(v.b * 2.0,
                   vec3(0.7, sin(t * 1.1) * 0.2, 0.0),
                   vec3(0.3, 0.2, cos(t * 1.2)),
                   vec3(sin(t * 0.8), 0.2, 0.3),
                   vec3(0.2, 0.2, cos(t * 0.9)));
  final.r = smoothstep(0.1, 0.91, length(v));
